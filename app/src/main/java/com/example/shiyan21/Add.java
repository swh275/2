    ...
    public void onClick(View v){  //取消加入按钮、保存按钮、插入图片按钮的点击事件
        ...
        switch (v.getId()) {
            case R.id.ButtonAddSave:
                ...
                @SuppressLint("SimpleDateFormat") DateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//配置时间格式
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 1) {
            if (resultCode == RESULT_OK) {
                imgP = Objects.requireNonNull(data).getStringExtra("imgP");
