    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        login.setOnClickListener(v -> {
            ...
            for(int i=0;i<message.length();i++){
                if(message.charAt(i)==','){
                    account.append(message.substring(j, i));
                    j=i;
                }
                if(message.charAt(i)=='.'){
                    password.append(message.substring(j, i));
                    j=i;
                }
            }
            password = new StringBuilder(password.substring(1));

            String[] n1= account.toString().split("\\.");
            String[] p1= password.toString().split("\\,");
            boolean flag=false;
        ...
        create.setOnClickListener(v -> {  //登陆界面监听器
